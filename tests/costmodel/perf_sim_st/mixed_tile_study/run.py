...
# One-command reproduce for the MIXED cube+vector tile cost-model study:
#   generate testcases -> build under __COSTMODEL -> run -> analyze.
#
# Usage:
#   python run.py                  # all experiments, full build + analyze
#   python run.py mixed_overlap    # one experiment
#   python run.py --no-build       # re-analyze existing CSVs only
#
# Requires: cmake, a C++23 compiler, GTest. No NPU -- the kernels build under -D__COSTMODEL and
# run the host pipeline simulator. The testcase names produced by experiments.py must be
